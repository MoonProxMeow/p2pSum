/* ============================================================
   AstralAurora v6.0 — P2P E2EE Chat
   Changes from NexTalk v5.1:
   • Rebranded to AstralAurora
   • Peer heartbeat/keepalive — mesh connections stay alive reliably
   • Caller priority (glare resolution) — no more double-call race condition
   • First-person call fix — track replacement when joining active calls
   • Noise isolation fix — conservative compressor; no double-processing
   • Custom emoji sync — peers share emoji packs after handshake
   • Friends stored inside profile JSON — one source of truth
   • Friend popup — message, call, remove, view profile actions
   • Latency — parallel post-handshake sends; fewer setTimeout chains
============================================================ */
"use strict";
...
const settingsManager = (() => {
  const KEY = "astral_settings";
  const _defaults = {
    _stripImageMetadata:true, _randomFileIds:true, _paddedEncryption:true,
    rehandshakeMinutes:10,
    adaptiveBitrate:true, voiceIsolation:false, pushToTalk:false, pttKey:" ",
    inlineMediaDefault:true, showReactions:true
  };
  let _s = { ..._defaults };
  function load() {
    try {
      const raw = localStorage.getItem(KEY) || localStorage.getItem("nextalk_settings");
...
/* ============================================================ ACCOUNT MANAGER
   Friends are now persisted inside the account JSON (acc.friends[]).
   Legacy separate storage key is migrated on first load.
============================================================ */
const accountManager = (() => {
  const KEY = "astral_account";
  let _acc = null;

  function _def() {
    return { username:"Anonymous", avatar:null, preferences:{}, customEmojis:{}, friendKey:_genKey(), friends:[] };
  }
  function _genKey() {
  ...
  function load() {
    try {
      const raw = localStorage.getItem(KEY) || localStorage.getItem("nextalk_account");
      if (raw) { const p = JSON.parse(raw); delete p.peerId; _acc = { ..._def(), ...p }; }
      else _acc = _def();
    } catch { _acc = _def(); }
    if (!_acc.friendKey) _acc.friendKey = _genKey();
    // Migrate friends from legacy separate key
    if (!Array.isArray(_acc.friends) || _acc.friends.length === 0) {
      try {
        const lf = JSON.parse(localStorage.getItem("astral_friends") || localStorage.getItem("nextalk_friends") || "[]");
        if (lf.length) _acc.friends = lf;
      } catch {}
    }
    if (!Array.isArray(_acc.friends)) _acc.friends = [];
    // Migrate custom emojis
    if (!_acc.customEmojis || !Object.keys(_acc.customEmojis).length) {
      try {
        const le = JSON.parse(localStorage.getItem("nextalk_custom_emojis") || "{}");
        if (Object.keys(le).length) _acc.customEmojis = le;
      } catch {}
    }
    if (!_acc.customEmojis) _acc.customEmojis = {};
    return _acc;
  }
  ...
  function exportJSON() {
    ...
    a.download = "astraAurora-account.json"; a.click(); URL.revokeObjectURL(a.href);
  }
  ...
  // Friends stored inside _acc.friends
  function getFriends()   { return Array.isArray(_acc?.friends) ? _acc.friends : []; }
  function saveFriends(f) { _acc.friends = f; localStorage.setItem(KEY, JSON.stringify(_acc)); }
  function addFriend(friendKey, name) {
    ...
    try {
      for (const pid of Object.keys(peerManager.getConnections())) {
  ...
  function getCustomEmojis()   { try { return _acc?.customEmojis || {}; } catch { return {}; } }
...
const encryptionManager = (() => {
  ...
  async function _ratchet(key) {
    ...
    return crypto.subtle.deriveKey(
      {name:"HKDF", hash:"SHA-256", salt:new Uint8Array(32), info:new TextEncoder().encode("AstralAurora-ratchet-v1")},
  ...
  async function setSession(pid, dhBytes, sigBytes) {
    ...
    const base = await crypto.subtle.importKey("raw", raw, "HKDF", false, ["deriveKey"]);
    const infoSend = myId < pid ? "AstralAurora-Stream-A" : "AstralAurora-Stream-B";
    const infoRecv = myId < pid ? "AstralAurora-Stream-B" : "AstralAurora-Stream-A";
    const [sendKey, recvKey] = await Promise.all([
      crypto.subtle.deriveKey({name:"HKDF",hash:"SHA-256",salt:new Uint8Array(32),info:new TextEncoder().encode(infoSend)}, base, {name:"AES-GCM",length:256}, true, ["encrypt","decrypt"]),
      crypto.subtle.deriveKey({name:"HKDF",hash:"SHA-256",salt:new Uint8Array(32),info:new TextEncoder().encode(infoRecv)}, base, {name:"AES-GCM",length:256}, true, ["encrypt","decrypt"])
    ]);
  ...
  async function encrypt(pid, obj) {
    ...
    return { iv:btoa(String.fromCharCode(...iv)), data:btoa(String.fromCharCode(...cb)), sig:btoa(String.fromCharCode(...new Uint8Array(sigBuf))), counter };
  }
...
const filePermissionManager = (() => {
  ...
  function handleRequest(fromPeer, {tokenId, fileName}) {
    ...
    ov.innerHTML = `<div class="perm-dialog"><div class="perm-icon">📥</div><div class="perm-title">Download Request</div><div class="perm-msg"><strong>${_e(name)}</strong> wants to download <em>${_e(fileName)}</em></div><div class="perm-btns"><button class="btn btn-primary btn-sm pa">Allow</button><button class="btn pd" style="background:var(--accent-red);color:#fff">Deny</button></div></div>`;
...
const peerManager = (() => {
  ...
  function _reconnectFriends() {
    accountManager.getFriends().forEach(f => {
      if (!f.lastPeerId || f.lastPeerId === _myId) return;
  ...
  function _startHeartbeat(pid) {
    _stopHeartbeat(pid);
    _hbTimers[pid] = setInterval(() => {
      const conn = _conns[pid];
      if (!conn || !conn.open) { _stopHeartbeat(pid); return; }
      if (Date.now() - (_lastPong[pid] || 0) > HB_TIMEOUT) {
        console.warn("AstralAurora: HB timeout", pid);
        _stopHeartbeat(pid); _teardown(pid);
        // Try to reconnect if this was a known friend
        setTimeout(() => { if (!_conns[pid]) connectTo(pid); }, 3000);
        return;
      }
      conn.send({type:"ping", ts:Date.now()});
    }, HB_INTERVAL);
  }
  function _stopHeartbeat(pid) {
    clearInterval(_hbTimers[pid]); delete _hbTimers[pid]; delete _lastPong[pid];
  }

  async function _handleRaw(conn, raw) {
    const pid = conn.peer;
    if (raw.type === "ping") { conn.send({type:"pong", ts:raw.ts}); return; }
    if (raw.type === "pong") { _lastPong[pid] = Date.now(); return; }
  ...
  function emergencyDisconnect() {
    Object.entries(_conns).forEach(([pid, c]) => { _stopHeartbeat(pid); try { c.close(); } catch {} });
...
/* ============================================================ CALL MANAGER
   FIX: caller priority — lower peer ID always initiates. Higher-ID waits 1.5s
        as fallback in case the lower-ID peer is not in-call yet.
   FIX: on startCall(), replace tracks in calls answered with empty MediaStream
        so the first user's mic goes out immediately.
   FIX: noise isolation — conservative compressor + gain trim prevents background
        noise amplification that occurred with the old high ratio/threshold.
============================================================ */
const callManager = (() => {
  const _calls = {};
  ...
  async function startCall() {
    ...
    // FIX: if peers already answered with an empty stream, push our real tracks in
    const lsAudio = _ls.getAudioTracks()[0];
    const lsVideo = _ls.getVideoTracks()[0];
    Object.values(_calls).forEach(call => {
      const pc = call.peerConnection; if (!pc) return;
      pc.getSenders().forEach(s => {
        if (s.track?.kind === "audio" && lsAudio) s.replaceTrack(lsAudio).catch(() => {});
        if (s.track?.kind === "video" && lsVideo) s.replaceTrack(lsVideo).catch(() => {});
      });
    });

    Object.keys(peerManager.getConnections()).forEach(pid => callPeer(pid));
    peerManager.broadcast({type:"call-state",inCall:true,cameraOn:false,muted:false,screenShare:false});
    _setupPTT();
    uiController.toast("Joined voice.");
  }

  /* FIX: noise isolation
     - HP at 100 Hz (not 80) with gentler Q to avoid resonance
     - LP at 7.5 kHz keeps voice clarity
     - Compressor threshold -15 dB (only peaks), ratio 3:1 (gentle)
     - Gain node 0.88 compensates for any makeup gain so output isn't louder than input */
  async function _applyVoiceIsolation(enable) {
    ...
    try {
      if (enable && !_audioCtx) {
        ...
        const hp   = _audioCtx.createBiquadFilter(); hp.type="highpass"; hp.frequency.value=100; hp.Q.value=0.5;
        const lp   = _audioCtx.createBiquadFilter(); lp.type="lowpass";  lp.frequency.value=7500; lp.Q.value=0.5;
        const comp = _audioCtx.createDynamicsCompressor();
        comp.threshold.value=-15; comp.knee.value=8; comp.ratio.value=3;
        comp.attack.value=0.003; comp.release.value=0.25;
        const gain = _audioCtx.createGain(); gain.gain.value=0.88;
        ...
        if (_filteredTrack) _replaceAudioInCalls(_filteredTrack);
  ...
  /* FIX: lower ID initiates; higher ID waits 1.5s then falls back */
  function callPeer(pid) {
    if (!_ls || _calls[pid]) return;
    const myId = peerManager.getId(); if (!myId) return;
    if (myId > pid) {
      setTimeout(() => { if (!_calls[pid] && _inCall && _ls) _doCallPeer(pid); }, 1500);
    } else {
      _doCallPeer(pid);
    }
  }
  function _doCallPeer(pid) {
  ...
  /* FIX: glare resolution — lower ID's outgoing call wins */
  function handleIncomingCall(call) {
    const myId = peerManager.getId() || "";
    if (_calls[call.peer]) {
      if (myId < call.peer) { try { call.close(); } catch {} return; } // our call wins
  ...
  function _setupCall(call) {
    ...
    call.on("error",  e      => { console.warn("Call:", e); _removeVideoEl(call.peer); });
  }
...
const fileTransferManager = (() => {
  ...
  function receiveEnd(data, pid) {
    ...
    if (dmManager.getActivePid()===pid||dmManager.hasDMWith(pid)) { dmManager.receiveFile(pid,meta,blob); }
    else uiController.appendFileMessage(profile.username||"Peer",meta.name,meta.size,blob,profile.avatar||null,pid,{oneTime:meta.oneTime,spoiler:meta.spoiler,expiry:meta.expiry});
    delete _in[data.id];
  }
...
const dmManager = (() => {
  ...
  document.getElementById("dm-emoji-btn").addEventListener("click", e => {
    ...
    picker.style.bottom=(window.innerHeight-r.top+6)+"px"; picker.style.left=r.left+"px";
    picker.classList.toggle("hidden"); e.stopPropagation();
  });
  dmImgInput.addEventListener("change", async function() {
    ...
    reader.onload=async e=>{
      ...
      _store(_activePid,{...msg,isSelf:true,isImg:true}); _renderImg({...msg,isSelf:true});
  ...
  async function send() {
    ...
    _store(_activePid,{...msg,isSelf:true}); _renderDM({...msg,isSelf:true});
    await peerManager.sendTo(_activePid,msg);
  }
  ...
  function receiveImage(fromPid,data) {
    const profile=peerManager.getProfile(fromPid); const msg={...data,isSelf:false,avatar:profile.avatar,isImg:true};
    _store(fromPid,msg);
  ...
  function _renderImg(msg){
    ...
    img.src=`data:${msg.mimeType};base64,${msg.b64}`; img.alt="Image";
  ...
  function _notifDot(pid){document.getElementById("nav-dms-badge")?.classList.remove("hidden");_renderConvList();}
  function _updateDMBadge(){
    ...
    if(total>0){badge.textContent=total>9?"9+":total;badge.classList.remove("hidden");}else badge.classList.add("hidden");
  }
  function _navToDMs(){
  ...
  function _renderConvList(){
    ...
    if(hint)hint.style.display="none"; list.innerHTML="";
    pids.forEach(pid=>{
      const profile=peerManager.getProfile(pid); const name=profile.username||pid.slice(0,8);
      const msgs=_hist[pid]; const last=msgs[msgs.length-1]; const unread=_unread[pid]||0;
...
const friendsManager = (() => {
  ...
  function reconcileConnectedPeers(){
    const conns=peerManager.getConnections();
    Object.entries(conns).forEach(([pid])=>{
      const p=peerManager.getProfile(pid); if(p.friendKey)onPeerConnected(pid,p);
    });
  }
  ...
  function _doRenderFriends(){
    ...
    const friends=accountManager.getFriends(); const conns=peerManager.getConnections();
    ...
    friends.forEach(f=>{
      ...
      if(isOnline&&f.lastPeerId){
        [avi,lbl].forEach(el=>{
          ...
          // Open the popup which now includes call + remove friend
      ...
      item.addEventListener("click",e=>{if(!isOnline||!f.lastPeerId)return;if(e.target.closest("button"))return;dmManager.open(f.lastPeerId);});
      list.appendChild(item);
    });
  }

  function sendRequest(pid){
    const acc=accountManager.get(); const token=crypto.randomUUID();
  ...
  function handleIncomingRequest(fromPid,data){
    ...
    _inboundQueue.push({fromPid,data}); _renderFriendRequests(); _showNextRequestToast();
  }
  let _toastVisible=false;
  function _showNextRequestToast(){
    if(_toastVisible||!_inboundQueue.length)return; _toastVisible=true;
    ...
    document.getElementById("frq-accept").onclick=()=>{_acceptRequest(fromPid,data);_dismissToast(data.token);};
    document.getElementById("frq-decline").onclick=()=>{peerManager.sendTo(fromPid,{type:"friend-decline",token:data.token});_dismissToast(data.token);uiController.toast("Friend request declined.");};
  }
  function _dismissToast(token){
    ...
    _toastVisible=false; _renderFriendRequests(); setTimeout(_showNextRequestToast,400);
  }
  ...
  function _renderFriendRequests(){
    ...
    if(sec)sec.style.display="block"; if(cnt)cnt.textContent=n;
    if(navBadge){navBadge.textContent=n;navBadge.classList.remove("hidden");}
    list.innerHTML="";
    _inboundQueue.forEach(({fromPid,data})=>{
      const profile=peerManager.getProfile(fromPid); const name=data.senderName||profile.username||fromPid.slice(0,8);
      ...
      dec.onclick=e=>{e.stopPropagation();peerManager.sendTo(fromPid,{type:"friend-decline",token:data.token});_dismissToast(data.token);};
...
const embedRenderer = (() => {
  ...
  function _buildLinkCard(url){
    const card=document.createElement("a"); card.href=url; card.target="_blank"; card.rel="noopener noreferrer"; card.className="link-embed-card";
    const fav=document.createElement("img"); fav.className="link-embed-favicon";
    const fs=_faviconSrc(url); if(fs){fav.src=fs;fav.onerror=()=>{fav.style.display="none";};}else{fav.style.display="none";}
    ...
    const urlEl=document.createElement("div"); urlEl.className="link-embed-url"; urlEl.textContent=url.length>72?url.slice(0,69)+"…":url;
    info.append(dom,urlEl); card.append(fav,info); return card;
  }
  function buildEmbed(url){
    const yt=url.match(YT);
    if(yt){const f=document.createElement("iframe");f.src=`https://www.youtube.com/embed/${yt[1]}`;f.width="320";f.height="180";f.style.cssText="border:none;border-radius:12px;display:block;margin-top:6px";f.allow="accelerometer;autoplay;clipboard-write;encrypted-media;gyroscope;picture-in-picture";f.allowFullscreen=true;return f;}
    ...
    if(VID.test(url)){const v=document.createElement("video");v.src=url;v.controls=true;v.style.cssText="max-width:320px;border-radius:12px;display:block;margin-top:6px";return v;}
...
/* ============================================================ EMOJI MANAGER
   FIX: peers' custom emojis are received, cached in _remote, and rendered
   inline alongside local emojis. Local always takes precedence on name collisions.
============================================================ */
const emojiManager = (() => {
  const picker  = document.getElementById("emoji-picker");
  ...
  let _custom   = {};
  let _remote   = {}; // from peers — render-only, not persisted
  const _RECENT_KEY="astral_recent_emoji";
  const MAX_RECENT=16;
  ...
  /* FIX: receive remote emojis; size-guard to prevent oversized payloads */
  ...
  function _buildPicker(){
    ...
    const fi=document.createElement("input"); fi.type="file"; fi.accept="image/*"; fi.style.display="none";
      ...
      const r=new FileReader(); r.onload=e=>{
        ...
        // Broadcast new emoji to all peers immediately
        peerManager.broadcast({type:"emoji-sync",emojis:{[name]:e.target.result}});
        _buildPicker(); picker.classList.remove("hidden");
        const ct=picker.querySelector('.emoji-tab[data-cat="Custom"]'); if(ct)ct.click();
...
const settingsUI = (() => {
  ...
  function _toggle(key,label,desc,onChangeCb){
    ...
    const lb=document.createElement("div"); lb.className="setting-label"; lb.textContent=label; lw.appendChild(lb);
  ...
  function _render(){
    ...
    const pttRow=_toggle("pushToTalk","Enable push-to-talk","Mic stays muted unless PTT key held",v=>{if(callManager.isInCall()){callManager.refreshPTT();if(!v)callManager.refreshMuteState();}});
    ...
    const curKey=settingsManager.get("pttKey")||" "; keyBtn.textContent=curKey===" "?"[Space]":curKey;
    keyBtn.onclick=()=>{
...
const uiController = (() => {
  ...
  function _col(n){const cols=["#8b5cf6","#d946ef","#34d399","#fb923c","#f43f5e","#22d3ee","#a78bfa","#e879f9"];let h=0;for(let i=0;i<(n||"").length;i++)h=n.charCodeAt(i)+((h<<5)-h);return cols[Math.abs(h)%cols.length];}
  function _makeInitialsUrl(name,sz=64){
    const c=document.createElement("canvas"); c.width=c.height=sz;
    const x=c.getContext("2d");
    // Radial gradient background for each avatar
    const grad=x.createRadialGradient(sz/2,sz/2,0,sz/2,sz/2,sz/2);
    const col=_col(name);
    grad.addColorStop(0,col+"cc"); grad.addColorStop(1,col+"66");
    x.beginPath(); x.arc(sz/2,sz/2,sz/2,0,Math.PI*2); x.fillStyle=grad; x.fill();
    x.fillStyle="#fff"; x.font=`700 ${Math.round(sz*.38)}px Syne,sans-serif`; x.textAlign="center"; x.textBaseline="middle";
  ...
  /* Friend popup — now includes Call and Remove Friend actions */
  function showProfilePopup(pid,anchorEl){
    ...
    // Call
    const callBtn=document.createElement("button"); callBtn.className="btn btn-secondary btn-sm"; callBtn.textContent="🎙 Call";
    callBtn.onclick=()=>{pp.classList.add("hidden");if(!callManager.isInCall()){callManager.startCall();}else{callManager.callPeer(pid);}};
    ...
    if(!isFriend){
      ...
      // Remove friend
      const rmBtn=document.createElement("button"); rmBtn.className="btn btn-sm";
      rmBtn.style.cssText="background:var(--accent-red);color:#fff";
      rmBtn.textContent="🚫 Remove Friend";
      rmBtn.onclick=()=>{if(confirm("Remove this friend?")){accountManager.removeFriend(profile.friendKey);friendsManager.renderFriends();pp.classList.add("hidden");uiController.toast("Friend removed.");}};
      actions.appendChild(rmBtn);
    }

    const rect=anchorEl?.getBoundingClientRect();
    if(rect){const top=Math.min(rect.bottom+8,window.innerHeight-300);pp.style.top=top+"px";pp.style.left=Math.min(rect.left,window.innerWidth-280)+"px";}
  ...
  function _appendMsgEl(container,{user,text,avatar,isSelf,isDM,msgId}){
    ...
    if(msgId){
      ...
      if(isDM||container===document.getElementById("dm-messages")) _dmMsgEls[msgId]=wrap;
      else _msgEls[msgId]=wrap;
    }
  ...
  function _applyReaction(rr,emoji,isSelf,msgId){
    ...
    if(pill){const cnt=pill.querySelector(".reaction-count");cnt.textContent=parseInt(cnt.textContent||"1")+1;if(isSelf)pill.classList.add("reacted");}
    else{
...
function showFileSendDialog(file,callback){
  ...
  document.getElementById("fcd-filename").textContent=file.name+" ("+_szG(file.size)+")";
  ...
  if(file.type.startsWith("image/")){
    const img=document.createElement("img"); img.style.cssText="max-width:200px;max-height:120px;border-radius:10px;object-fit:cover";
...
function _szG(b){if(!b)return"";if(b<1024)return b+" B";if(b<1048576)return(b/1024).toFixed(1)+" KB";return(b/1048576).toFixed(1)+" MB";}
...
(function wireEvents(){
  ...
  document.addEventListener("click",e=>{const pp=document.getElementById("profile-popup");if(pp&&!pp.classList.contains("hidden")&&!pp.contains(e.target))pp.classList.add("hidden");});
  document.getElementById("emergency-disconnect")?.addEventListener("click",()=>{if(!confirm("☠ Emergency disconnect?"))return;peerManager.emergencyDisconnect();});
  document.getElementById("copy-id")?.addEventListener("click",()=>navigator.clipboard.writeText(document.getElementById("my-id").textContent).then(()=>uiController.toast("Node ID copied!")));
  document.getElementById("host")?.addEventListener("click",()=>{const un=document.getElementById("username")?.value.trim();if(un){accountManager.save({username:un});uiController.initAvatarDisplay();}uiController.appendSystemMessage("Room created — share your Node ID");uiController.toast("Room ready!");});
  document.getElementById("join")?.addEventListener("click",()=>{const un=document.getElementById("username")?.value.trim();if(un){accountManager.save({username:un});uiController.initAvatarDisplay();}const hostId=document.getElementById("host-id").value.trim();if(!hostId){uiController.toast("Paste a peer ID first.");return;}peerManager.connectTo(hostId);uiController.appendSystemMessage("Connecting to peer…");});
  document.getElementById("host-id")?.addEventListener("keydown",e=>{if(e.key==="Enter")document.getElementById("join")?.click();});
  document.getElementById("username")?.addEventListener("input",e=>{const acc=accountManager.get();uiController._applyAvatar(document.getElementById("my-avatar"),e.target.value||"?",acc.avatar);uiController._applyAvatar(document.getElementById("my-avatar-mini"),e.target.value||"?",acc.avatar);document.getElementById("my-username-display").textContent=e.target.value||"Anonymous";});
  document.getElementById("acc-save")?.addEventListener("click",()=>{const un=document.getElementById("username")?.value.trim()||"Anonymous";accountManager.save({username:un});uiController.initAvatarDisplay();uiController.toast("Account saved!");peerManager.broadcast({type:"profile",user:un,avatar:accountManager.get().avatar,friendKey:accountManager.get().friendKey});});
  document.getElementById("acc-export")?.addEventListener("click",()=>accountManager.exportJSON());
  document.getElementById("acc-import")?.addEventListener("change",async function(){const f=this.files[0];if(!f)return;this.value="";try{const acc=await accountManager.importJSON(f);uiController.initAvatarDisplay();emojiManager.init();uiController.toast(`Account loaded: ${acc.username}`);}catch(e){uiController.toast("Import failed: "+e.message);}});
  document.getElementById("acc-burn")?.addEventListener("click",()=>{if(!confirm("🔥 Burn account? Deletes all local data."))return;accountManager.burnAccount();settingsManager.clearAll();uiController.initAvatarDisplay();friendsManager.renderFriends();uiController.toast("Account burned.");});
  ...
  document.getElementById("ptt-btn")?.addEventListener("click",()=>{settingsManager.set("pushToTalk",!settingsManager.get("pushToTalk"));uiController.updateCallUI(callManager.isInCall(),false,false,false,false);uiController.toast(settingsManager.get("pushToTalk")?"PTT ON (hold bound key)":"PTT OFF");if(callManager.isInCall())callManager.refreshPTT();});
  ...
  function _sendChat(){
    ...
    input.value="";
    const acc=accountManager.get(); const msgId=crypto.randomUUID(); const ts=Date.now();
    uiController.appendMessage({user:acc.username,text,avatar:acc.avatar,isSelf:true,msgId,ts});
    peerManager.broadcast({type:"message",channel:"general",user:acc.username,text,msgId,ts});
  }
  ...
  document.addEventListener("keydown",e=>{
    if(e.key==="Escape"){document.getElementById("lightbox")?.classList.add("hidden");document.querySelectorAll(".reaction-picker-popup").forEach(p=>p.remove());}
  });
})();
...
(function init(){
  ...
  uiController.updateCallUI(false,false,false,false,false);
  uiController.appendSystemMessage("AstralAurora started — create a room or join a peer");
})();
